...
const TextArea: React.FC<TextAreaProps> = memo((props) => {
  ...
  const { attr } = props;
  return (
    <View style={{ marginTop: 10, height:attr.multiline ? 120 : 55,backgroundColor:colors.white,borderRadius: 10, }}>
      <Grid style={[styles.searchInputHolder,{height:attr.multiline ? 120 : 55}]}>
        <Col size={0.15} style={{ justifyContent: 'center', alignItems: 'center',marginLeft:5 }}>
          <Icon name={attr.icon.name} type={attr.icon.type as any} color={attr.icon.color} size={24} />
        </Col>
